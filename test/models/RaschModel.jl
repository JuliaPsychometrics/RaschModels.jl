@testset "RaschModel" begin
    ...
    @testset "iif" begin
        @test RaschModels._iif(RaschModel, 0.0, 0.0, 0) ==
              RaschModels._iif(RaschModel, 0.0, 0.0, 1)
    ...
    @testset "expected_score" begin
        ...
        @test expected_score(model_mcmc, 0.0, 1, scoring_function = log) ==
              log.(irf(model_mcmc, 0.0, 1))
        @test expected_score(model_mcmc, 0.0, 1:2, scoring_function = log) ==
              log.(irf(model_mcmc, 0.0, 1)) .+ log.(irf(model_mcmc, 0.0, 2))
        ...
        @test expected_score(model_mle, 0.0, 1; scoring_function = log) ==
              log(irf(model_mle, 0.0, 1))
        @test expected_score(model_mle, 0.0, 1:2, scoring_function = log) ==
              log(irf(model_mle, 0.0, 1)) .+ log(irf(model_mle, 0.0, 2))
    end
    ...
    @testset "information" begin
        ...
        @test information(model_mcmc, 0.0, 1, scoring_function = log) ==
              log.(iif(model_mcmc, 0.0, 1))
        @test information(model_mcmc, 0.0, 1:2, scoring_function = log) ==
              log.(iif(model_mcmc, 0.0, 1)) .+ log.(iif(model_mcmc, 0.0, 2))
        ...
        @test information(model_mle, 0.0, 1, scoring_function = log) ==
              log(iif(model_mle, 0.0, 1))
        @test information(model_mle, 0.0, 1:2, scoring_function = log) ==
              log(iif(model_mle, 0.0, 1)) .+ log(iif(model_mle, 0.0, 2))
    end
end
